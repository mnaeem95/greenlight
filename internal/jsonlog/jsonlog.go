...
// Define a custom Logger type. This holds the output destination that the log entries will be written to,
// the minimum severity level that log entries will be written for, plus a mutex for coordinating the writes.
type Logger struct {
	out      io.Writer
	minLevel Level
	mu       sync.Mutex
...
func (l *Logger) print(level Level, message string, properties map[string]string) (int, error) {
	...
	// Lock the mutex so that no two writes to the output destination cannot happen concurrently. If we don't do this, it's possible that
	// the text for two or more log entries will be intermingled in the output.
	l.mu.Lock()
	defer l.mu.Unlock()
